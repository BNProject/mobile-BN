...
class DashboardScreen extends Component {
  ...
  async componentDidUpdate(prevProps, prevState) {
    if (this.state.cari !== prevState.cari) {
      if (this.state.cari === '') {
        ...
        let hasilSearch = await this.state.data.filter(
          el =>
            el.tbl_store.store_code
              .toLowerCase()
              .match(new RegExp(this.state.cari.toLowerCase())) ||
            el.tbl_store.store_name
              .toLowerCase()
              .match(new RegExp(this.state.cari.toLowerCase())) ||
            el.tbl_store.tbl_retailer.initial
              .toLowerCase()
              .match(new RegExp(this.state.cari.toLowerCase())),
        );
  ...
  fetchDataLoadMore = async () => {
    try {
      let token = await AsyncStorage.getItem('token_bhn_md');
      let getData = await API.get(`/visit?page=${this.state.page}`, {
        headers: {token},
      });
      let hasilSearch;

      if (this.state.cari !== '') {
        hasilSearch = await getData.data.data.filter(
          el =>
            el.tbl_store.store_code
              .toLowerCase()
              .match(new RegExp(this.state.cari.toLowerCase())) ||
  ...
  navigateListPendingVisit = () => {
    this.props.navigation.navigate('ListPendingVisit', {
      sendPendingVisit: () => this.sendPendingVisit(),
    });
  };
      ...
      <>
        <SafeAreaView
          ...
          <ScrollView
            ...
            {this.state.loading ? (
              ...
              this.state.dataForDisplay.map((element, index) => (
                <TouchableHighlight
                  ...
                  underlayColor="white"
                  onPress={() =>
                    this.props.navigation.navigate('DetailVisit', {
                      data: element,
                    })
                  }>
            ...
            {this.state.loadingText && this.state.data.length > 0 && (
              <Text style={{alignSelf: 'center', margin: 5, color: 'white'}}>
                Loading more...
              </Text>
            )}
          </ScrollView>
          {this.state.sendPendingVisit && (
            <Fab
              ...
              <Image
                source={require('../asset/pending.png')}
                style={{
                  height: 35,
                  width: 35,
                  resizeMode: 'stretch',
        ...
        <Popover
          ...
          <List>
            {this.state.dataNotification.map(element => (
              <ListItem
                selected
                key={`${element.id}${element.path_file}`}
                onPress={() =>
